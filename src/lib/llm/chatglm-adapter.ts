...
export class ChatGLMAdapter implements BaseLLMAdapter {
  private apiKey: string;
  private model: string;
  private endpoint: string;

  constructor(apiKey: string, model: string = "glm-4v", endpoint: string = "https://open.bigmodel.cn/api/paas/v4") {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private async callAPI(messages: any[]): Promise<LLMResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`智谱AI API 调用失败: ${error}`);
    }
