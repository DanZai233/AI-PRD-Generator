...
export class YiAdapter implements BaseLLMAdapter {
  private apiKey: string;
  private model: string;
  private endpoint: string;

  constructor(apiKey: string, model: string = "yi-vision", endpoint: string = "https://api.lingyiwanwu.com/v1") {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private async callAPI(messages: any[]): Promise<LLMResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`零一万物 API 调用失败: ${error}`);
    }
