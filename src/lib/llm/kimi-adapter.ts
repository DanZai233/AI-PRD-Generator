...
export class KimiAdapter implements BaseLLMAdapter {
  private apiKey: string;
  private model: string;
  private endpoint: string;

  constructor(apiKey: string, model: string = "moonshot-v1-8k-vision", endpoint: string = "https://api.moonshot.cn/v1") {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private async callAPI(messages: any[]): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Kimi API 调用失败: ${error}`);
    }
