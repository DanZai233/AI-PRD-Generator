...
export class DoubaoAdapter implements BaseLLMAdapter {
  private apiKey: string;
  private model: string;
  private endpoint: string;

  constructor(apiKey: string, model: string = "doubao-pro-4k", endpoint: string = "https://ark.cn-beijing.volces.com/api/v3") {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private async callAPI(messages: any[]): Promise<LLMResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`火山引擎 API 调用失败: ${error}`);
    }
